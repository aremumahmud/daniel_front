...
export async function GET(request: NextRequest) {
  try {
    ...
    // Filter patients based on assignments
    let filteredPatients = Array.from(mockPatients.values()).filter((patient: any) => 
    ...
    if (status && status !== 'all') {
      filteredPatients = filteredPatients.filter((patient: any) => {
        if (status === 'active') return patient.isActive
        if (status === 'inactive') return !patient.isActive
        return true
      })
    }
    ...
    filteredPatients.sort((a: any, b: any) => {
      ...
      switch (sortBy) {
        ...
        case 'age':
          ...
          break
        case 'lastLogin':
          aValue = new Date(a.userId?.lastLogin || 0).getTime()
          bValue = new Date(b.userId?.lastLogin || 0).getTime()
          break
        default:
